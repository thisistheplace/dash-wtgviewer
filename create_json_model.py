...
def read_model(json_path: str | Path):
    ...
    for node in data["nodes"].values():
        node = Node(x=node["X"], y=node["Y"], z=node["Z"])
        nodes[node.id] = node
    max_node_id = max(list(nodes.keys()))
    # extra hub node
    nodes[max_node_id + 1] = Node(
        x=nodes[max_node_id].x - 1.5, y=nodes[max_node_id].y, z=nodes[max_node_id].z
    )

    node_sets["foundation"] = NodeSet(
        name="foundation", nodes=[nodes[nid] for nid in range(1, 8)]
    )

    node_sets["tower"] = NodeSet(
        name="tower", nodes=[nodes[nid] for nid in range(8, 11)]
    )
    ...
    element_sets["foundation"] = ElementSet(
        name="foundation", elements=[elements[elid] for elid in range(1, 7)]
    )

    element_sets["tower"] = ElementSet(
        name="tower", elements=[elements[elid] for elid in range(7, 10)]
    )
    ...
    foundation = Foundation(
        node_sets=[node_sets["foundation"]], element_set=element_sets["foundation"]
    )
    hub = Hub(cone=Cone(nodes=list(nodes.values())[-2:], diameter=1.5))
    rotor = Rotor(blades=blades, hub=hub, node=list(nodes.values())[-2])
    nacelle = Nacelle(element=elements[10])
    tower = Tower(node_sets=[node_sets["tower"]], element_set=element_sets["tower"])
