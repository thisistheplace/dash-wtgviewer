...
/**
 * Copied from: 'three/examples/jsm/objects/Water'
 * Modified to work with an xy plane
 * Work based on :
...
class Water extends Mesh {
	...
	constructor( geometry: THREE.BufferGeometry, options: WaterOptions ) {
		...
		const mirrorShader = {
			...
			fragmentShader: /* glsl */`
				...
				void main() {
					...
					vec3 surfaceNormal = normalize( noise.xzy * vec3( 1.5, 1.5, 1.0 ) );
		...
		scope.onBeforeRender = function ( renderer, scene, camera: THREE.PerspectiveCamera | THREE.OrthographicCamera ) {
